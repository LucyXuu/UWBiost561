...
#'
#' @example adj <- generate_partial_clique(n = 10, clique_fraction = 0.5, clique_edge_density = 0.95)
#'          compute_maximal_partial_clique(adj, alpha=0.9)
#' @export
compute_maximal_partial_clique <- function(adj_mat, alpha) {
  ...
  for (nodei in 1:n) {
    ...
    current_density <- calculate_density(current_clique, adj_mat)
    while (current_density < alpha) {
      current_clique <- remove_one_node(current_clique, adj_mat)
      current_density <- calculate_density(current_clique, adj_mat)
    }
    ...
    for (nodej in connected_nodes) {
      ...
      if (length(nodej_clique) >= 1) {
        for (nodej1 in nodej_clique) {
          ...
          expanded_density <- calculate_density(expanded_clique, adj_mat)
          ...
          if (!list_match(ran_clique, current_clique)) {
  ...
  density <- calculate_density(maximal_clique, adj_mat)
...
calculate_density <- function(clique, adj_mat) {
  m <- length(clique)
  if (m <= 1){return(1)}
  num_edge <- 0
  for (i in 1:(m - 1)) {
...
remove_one_node <- function(clique, adj_mat) {
  densities <- sapply(clique, function(node) {
    temp_clique <- clique[clique != node]
    return(calculate_density(temp_clique, adj_mat))
  })
...
list_match <- function(ran_clique, clique) {
