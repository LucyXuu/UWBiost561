...
output: 
 html_document:
   output_dir: "C:/Users/plum/OneDrive/Desktop/BIOST561/UWBiost561/vignettes"
vignette: >
...
mean_population <- country_population %>%
  ...
  summarize(mean = round(mean(population), 0))
...
```{r}
result <- country_birth %>%
  select(c(iso3c, year, life_expect)) %>%
  na.omit %>%
  group_by(iso3c)%>%
  summarize(min_life_expect = min(life_expect), max_life_expect = max(life_expect)) %>%
...
```{r}
inner_join(country_population, country_birth, by="iso3c")
```
...
It will generate a n*n adjacency matrix. Each column represents one node, and 1 at this column and ith row represent hos this node is connected to the ith node.
...
This function converts the adjacency matrix into the form where the first and second column represents the row and column name of the adjacency matrix, and the value column represents the actual value in that specific entry.
...
This heat map is closely related to the adjacency matrix, as if we put that matrix onto a graphing board, each row can column takes space of 1 square, and color every o to palegoldenrod and 1 as coral2, then we will get the above heat map.
...
When we formed the random adjacency matrix, we first created it by clique, and then messes it so the clique is not nodes next to each other. This reversed the messing up process and created this heatmap.
...
Sorry I didn't quite understand this question. I think I will just create adjacency matrixs with known maximal clique, and see if the function can return the correct answer. I may also create some matrixs with no connection to each other and see if the function correctly distinguish it, and this is all I can think of.
